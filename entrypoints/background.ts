...
export default defineBackground((context) => {
  ...
  // Helper function to fetch raw messages
  async function fetchRawMessages(agent: BskyAgent): Promise<any[]> {
    try {
      // Querying threads instead of direct message API since it's more reliable
  ...
  function sendWebSocketMessage(message: any) {
    try {
      if (wsConnection && wsConnection.readyState === WebSocket.OPEN) {
        ...
        console.error('Notisky: WebSocket not open, cannot send message');
      ...
      console.error('Notisky: Error sending WebSocket message', error);
      return false;
    }
  }
  ...
  async function setupWebSocketConnection() {
    try {
      ...
      // Reset connection attempts if we've cycled through all URLs
      if (wsConnectionAttempts >= MAX_RECONNECT_ATTEMPTS) {
        wsConnectionAttempts = 0;
        wsUrlIndex = (wsUrlIndex + 1) % ALTERNATE_WS_URLS.length; // Try next URL in rotation
      }
      
      // Initialize with fallback WebSocket URL
      let wsUrl = AUTH_SERVER_WS_URL;
      ...
      try {
        ...
        const connectionTimeout = setTimeout(() => {
          if (wsConnection && wsConnection.readyState !== WebSocket.OPEN) {
            console.error('Notisky: WebSocket connection timeout');
            wsConnection.close();
            wsConnection = null;
            
            // Attempt to reconnect with next URL
        ...
        wsConnection.onmessage = (event) => {
          try {
            ...
            if (data.type === 'accounts') {
              // New type for account credentials
              handleAccountsUpdate(data.accounts);
            } else if (data.type === 'connection') {
              // Just log connection messages
        ...
        wsConnection.onclose = (event) => {
          console.log(`Notisky: WebSocket connection closed, code: ${event.code}, reason: ${event.reason}`);
          wsConnection = null;
          ...
          if (wsConnectionAttempts < MAX_RECONNECT_ATTEMPTS) {
            ...
            console.log(`Notisky: Attempting to reconnect in ${delay / 1000} seconds (attempt ${wsConnectionAttempts} of ${MAX_RECONNECT_ATTEMPTS})`);
            
            wsReconnectTimer = setTimeout(() => {
              setupWebSocketConnection();
            }, delay) as unknown as number;
          } else {
            console.log('Notisky: Max reconnect attempts reached for current URL, trying alternative URL');
            // Reset connection attempts and try next URL
            wsUrlIndex = (wsUrlIndex + 1) % ALTERNATE_WS_URLS.length;
            wsConnectionAttempts = 0;
            
            // Try immediately with next URL
            wsReconnectTimer = setTimeout(() => {
              setupWebSocketConnection();
            }, 1000) as unknown as number;
          }
  ...
  // Handle accounts update (new function)
  async function handleAccountsUpdate(accounts: AccountSession[]) {
    if (!accounts || !Array.isArray(accounts) || accounts.length === 0) {
      console.log('Notisky: No account data received');
      return;
    }
    ...
    try {
      ...
      // Merge accounts, overriding existing ones with new data
      const mergedAccounts = [...existingAccounts];
      
      accounts.forEach(newAccount => {
        const existingIndex = mergedAccounts.findIndex(
          (acc: AccountSession) => acc.did === newAccount.did
        );
      ...
      // Initialize headless clients for new or updated accounts
      for (const account of accounts) {
        await initHeadlessClientForAccount(account);
      }
      
    } catch (error) {
      console.error('Notisky: Error handling accounts update', error);
    }
  }
